...
public class ConsultarVideo extends HttpServlet {
	...
	private void agregarConsultaHistorial(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String aux = request.getParameter("id");
			String duenioVideo = ws.findDuenioVideo(Integer.parseInt(aux));
			DtVideo v = null;
			v = ws.findVideo(Integer.parseInt(aux));
			ws.seleccionarUsuario(duenioVideo);
			ws.seleccionarVideo(v.getNombre());
			ws.seleccionarUsuario(nick);
			DtLista lst = ws.seleccionarLista("Historial");
			ws.agregarVideo(v.getNombre(), lst);
			ws.finCasoUso();
		} catch (Exception e) {
